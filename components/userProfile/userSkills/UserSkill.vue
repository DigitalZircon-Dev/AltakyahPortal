<template>
  <div>
    <v-chip
      :close="userSkill.isAllowedDelete"
      color="purple"
      text-color="white"
       class="ma-2"
      close-icon="mdi-close"
      @click:close="displayDeleteUserSkill(userSkill.id)"
    >
      <v-avatar left  v-if="userSkill.isAllowedEndorse">
        <v-btn
          icon
          @click.prevent="onEndorse(!isEndorse)"
        >
          <v-icon>{{ isEndorse ? 'mdi-check ' : 'mdi-plus' }}</v-icon>
        </v-btn>
      </v-avatar>
      {{ userSkill.skillName }}
      <span class="skillEndorses"
        v-if="countEndorse > 0"
        @click.prevent="
          displayUserSkillEndorses(userSkill.id, userSkill.skillName)
        "
        >({{ countEndorse }})</span
      >
    </v-chip>
...
import UserSkillEndorse from '../userSkills/UserSkillEndorse'
